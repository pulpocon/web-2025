<template>
  <section class="sps-section" id="sponsors">
    <div class="container-fluid-1440">
      <header class="sps-section__header">
        <h2 class="sps-section__title" data-aos="fade-up">Sponsors</h2>
        <p class="sps-section__description" data-aos="fade-up" data-aos-delay="200">
          #pulpoCon25 is possible thanks to the support of our sponsors. We are grateful for their commitment to the
          community and their contribution to the success of this event.
        </p>
      </header>

      <div class="sps-section__sps">
        <sponsors></sponsors>
      </div>
      <div class="sps-section__sps" style="text-align: center">
        <header class="sps-section__header">
          <h3 class="sps-section__title" style="font-size:2rem">Sponsor #pulpoCon25</h3>
        </header>
        <a class="btn"
           href="https://docs.google.com/presentation/d/1pRGA1fH5ULCLFxAamWO92UqcpQc1iEQjOMT2zw6kgpU/edit?usp=sharing"
           target="_blank">
          Sponsors Deck
        </a>
      </div>
    </div>
  </section>
</template>
<script lang="ts" setup>
import Sponsors from '~/components/Sponsors'

defineProps()

defineEmits()

</script>
<style src="./SponsorsSection.scss"></style>
